...
def provision_new_certificate(domain=None):
    ...
    all_our_domains = { env['PRIMARY_HOSTNAME'] } | set(web['domain'] for web in mailinabox_management_api("/web/domains", is_json=True))

    if domain is None:
        # Install a server certificate for all domains hosted by the box.
        install_domain = env['PRIMARY_HOSTNAME']
        domains = all_our_domains
    elif domain not in all_our_domains:
        raise ValueError("That's not a domain hosted here.")
    else:
        # Request a cert for this domain and any subdomains that we're also serving.
        install_domain = domain
        domains = { d for d in all_our_domains if d == domain or d.endswith("." + domain) }
    ...
    # Sort to put the install domain first, so that it becomes the CN.
    # If we're requesting a system-wide certificate, that'll be the PRIMARY_HOSTNAME.
    # If we're asking for a certificate for a particular domain, we want that domain
    # to be the common name.
    domains = sorted(domains, key = lambda x : x != install_domain)
        ...
        try:
            ...
            for action in e.actions:
                if isinstance(action, client.NeedToInstallFile):
                    print("Installing", os.path.join(challenges_path, action.file_name))
            ...
            if ret in ("OK", "OK\nweb updated"): # sadly our API gives a human-readable response only
...
if __name__ == "__main__":
    provision_new_certificate(None if len(sys.argv) <= 1 else sys.argv[1])
